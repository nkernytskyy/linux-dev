 ...
 *
 * Compile with `make`. Load with `sudo insmod chardev.ko`. 
 * Check `dmesg | tail` output to see the assigned device number 
 * and command to create a device file.
 *
...
// Prototypes - normally go in a .h file
...
static void __exit clean_module(void) {
  printk(KERN_INFO "REMOVED Character device.\n");
...
static ssize_t device_write(
  struct file *filp, const char *buffer, size_t len, loff_t *offset)
{
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("(C) GlobalLogic OpSy Team");
MODULE_DESCRIPTION("A simple example of a Linux character device.");
MODULE_VERSION("0.01");
